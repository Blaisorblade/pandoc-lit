...
import Text.Pandoc.Definition (Pandoc (Pandoc), Block (Header), Inline (), nullAttr)
...
toStructure = fst . go 0
  ...
  go outer rest@(Header inner attrs _ : _) | inner <= outer
    =  ([], rest)

  go outer (Header inner attrs text : rest)
